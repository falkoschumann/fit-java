...
public class FrameworkTest extends TestCase {
    ...
    protected void run(String file, int right, int wrong, int ignores, int exceptions) throws Exception {
        String input = read(new File("src/main/examples/"+file+".html"));
        ...
        new File("target/output/test/").mkdirs();
        PrintWriter output = new PrintWriter(new BufferedWriter(new FileWriter("target/output/test/"+file+".html")));
        tables.print(output);
        output.close();
